...
public class ProductController {
    ...
    public String updateProduct(@Valid @ModelAttribute("product") Product incoming,
        ...
        if (incoming.getBrand() != null && incoming.getBrand().getBrandId() > 0) {
            current.setBrand(brandRepository.findById(incoming.getBrand().getBrandId()).orElseThrow());
        }
        if (current.getLifecycleStatus() != ProductLifecycleStatus.SELLING) {
...
//        }
        ...
        model.addAttribute("message", "Product has been discontinued and hidden successfully.");
        return "redirect:/staff/products/list";
    }
